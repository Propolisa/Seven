...
const htbCharts = require("./charts/index.js");
...
async function htbApiGet(endpointPath, token = API_TOKEN) {
  return new Promise((resolve, reject) => {
    const agent = request.agent();
    agent
...
async function getCurrentMachines() {
  return (await htbApiGet("machine/list", API_TOKEN)).info;
}

async function getRetiredMachines() {
  return (await htbApiGet("machine/list/retired", API_TOKEN)).info;
}
...
async function getCurrentChallenges() {
  return (await htbApiGet("challenge/list", API_TOKEN)).challenges;
}

async function getRetiredChallenges() {
  return (await htbApiGet("challenge/list/retired", API_TOKEN)).challenges;
}
...
async function getMemberProfile(memberId) {
  ...
  result = await htbApiGet(uri + memberId, API_TOKEN);
...
async function getMemberProfiles(memberIds) {
  ...
  memberIds.forEach((memberId) => {
    memberPromises.push(htbApiGet(uri + memberId, API_TOKEN));
  });
...
async function main() {
  ...
  API_TOKEN = await getV4Session(process.env.HTB_EMAIL, process.env.HTB_PASS,);
